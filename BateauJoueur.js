...
function BateauJoueur()
{
  this.images = null;
  this.imagecanon=null;
  
	this.initBateau = function(x,y)
	{
    this.init();
		this.initializeVisualObject(img_bateaugauche, x, y);
    this.nbLastCanon=2;
    this.nbTotalCanon=2;
    ...
    this.ecume.x = this.x-120;
    this.ecume.y = this.y+50;
    this.ecume.image = img_ecume_droit;
    this.sens=-1;
    //this.g_feucanon[3].initializeObject(this.x-61, this.y+50);
    //this.g_feucanon[2].initializeObject(this.x-53, this.y+62);
    for(var i=this.nbTotalCanon-1;i>=0;i--){
  ...
  /**
	 ...
	 */
    this.DrawJoueur = function(context)
    {
      context.globalAlpha=this.alpha;
      var nbimg = this.images.length;
      for(var i=0;i<nbimg;i++){
        context.drawImage(this.images[i], this.x , this.y);
      }
      for(var i=0;i<this.nbTotalCanon;i++){
        context.drawImage(this.imagecanon, this.x+98+13*i , this.y+133-i*5); 
      }
      context.globalAlpha=1;
    }  
}
BateauJoueur.prototype = new Bateau;
